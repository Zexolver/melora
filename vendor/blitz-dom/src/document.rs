...
impl BaseDocument {
    ...
    pub fn resolve(&mut self) {
        ...
        self.reparent_fixed_positioned_descendants();
    ...
    /// Splices every `position: fixed` element out of wherever
    /// `collect_layout_children` naturally placed it in the layout/paint
    /// tree and makes it a direct layout/paint child of the root element
    /// instead.
    ///
    /// Why this exists: Taffy's `Position::Absolute` (what stylo_taffy
    /// maps both CSS `position: fixed` and `position: absolute` to --
    /// Taffy has no separate "fixed" concept, see `stylo_taffy::convert`)
    /// is only ever resolved against a node's *immediate* layout parent --
    /// `perform_absolute_layout_on_absolute_children` and its
    /// flexbox/grid equivalents each only place their own direct
    /// absolutely-positioned children, with no mechanism to walk further
    /// up to the nearest genuinely positioned ancestor the way real CSS
    /// does. Concretely, that means a real `position: fixed` header
    /// nested a few levels deep under ordinary (`position: static`)
    /// wrapper `<div>`s -- extremely common markup, confirmed live on
    /// duckduckgo.com's header/sidebar -- was resolving relative to
    /// whichever div happened to be its immediate parent instead of the
    /// viewport, and paint's per-ancestor scroll-offset subtraction
    /// (`ElementCx`/`render_element`, blitz-paint's `render.rs`) made it
    /// scroll away with the page instead of staying put, since nothing
    /// distinguished it from an ordinary in-flow descendant once painting
    /// walked down through its real ancestor chain.
    ///
    /// Root-parenting a fixed element in the *same* `layout_children`/
    /// `paint_children` lists Taffy and paint both already walk fixes
    /// both problems in one step, for free: Taffy now resolves its
    /// `Position::Absolute` against the root (its only positioned
    /// ancestor once reparented), landing it at the correct viewport-
    /// relative offset; and paint reaches it directly from the root
    /// without passing through any intervening ancestor's scroll offset,
    /// so it naturally stays visually fixed as the page scrolls.
    ///
    /// True CSS `position: sticky` isn't attempted here -- unlike fixed,
    /// its containing block is normal (it reserves in-flow space and only
    /// clamps to an edge past a scroll threshold), so it doesn't have the
    /// same "wrong ancestor" bug; making it actually stick would mean
    /// recomputing its offset live against current scroll position on
    /// every paint, a materially bigger feature left as a known gap (see
    /// ARCHITECTURE.md).
    ///
    /// Runs unconditionally at the end of every `resolve_layout_children`
    /// call (full or damage-skipped) rather than being folded into
    /// `collect_layout_children`/`resolve_layout_children_recursive`
    /// themselves, specifically so it doesn't have to reason about that
    /// function's incremental-damage bookkeeping: it always re-derives
    /// which nodes are fixed from a fresh walk of the *stable* DOM
    /// `children` list (never mutated by layout), and always splices
    /// against whatever `layout_children`/`paint_children` currently
    /// contain (freshly recomputed or reused, doesn't matter) using
    /// `retain`/`contains` -- so re-running it on an already-reparented
    /// tree is a no-op, not a double-move.
    pub fn reparent_fixed_positioned_descendants(&mut self) {
        ...
        collect_fixed_positioned_ids(self, root_id, root_id, &mut fixed_ids);

        for &fixed_id in &fixed_ids {
            if let Some(parent_id) = self.nodes[fixed_id].layout_parent.get() {
                if parent_id != root_id {
                    if let Some(children) = self.nodes[parent_id].layout_children.borrow_mut().as_mut() {
                        children.retain(|&id| id != fixed_id);
                    }
                    if let Some(children) = self.nodes[parent_id].paint_children.borrow_mut().as_mut() {
                        children.retain(|&id| id != fixed_id);
                    }
                }
            }
            self.nodes[fixed_id].layout_parent.set(Some(root_id));
        }

        if let Some(children) = self.nodes[root_id].layout_children.borrow_mut().as_mut() {
            for &fixed_id in &fixed_ids {
                if !children.contains(&fixed_id) {
                    children.push(fixed_id);
                }
            }
        }
        if let Some(children) = self.nodes[root_id].paint_children.borrow_mut().as_mut() {
            for &fixed_id in &fixed_ids {
                if !children.contains(&fixed_id) {
                    children.push(fixed_id);
                }
        ...
        /// Walks the stable DOM `children` list (not `layout_children`,
        /// which is what's being rewritten here) to find every element
        /// with a computed `position: fixed`, root or not.
        fn collect_fixed_positioned_ids(
            doc: &BaseDocument,
            node_id: usize,
            root_id: usize,
            out: &mut Vec<usize>,
            ...
            for &child_id in &doc.nodes[node_id].children {
                if child_id != root_id {
                    let is_fixed = doc.nodes[child_id]
                        .primary_styles()
                        .is_some_and(|s| matches!(s.clone_position(), Position::Fixed));
                    if is_fixed {
                        out.push(child_id);
                    }
                }
                collect_fixed_positioned_ids(doc, child_id, root_id, out);
            }
        }
    }
