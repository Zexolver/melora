...
use style::{
    dom::TElement,
    properties::{
        ComputedValues, generated::longhands::position::computed_value::T as StyloPosition,
        generated::longhands::visibility::computed_value::T as StyloVisibility, style_structs::Font,
    },
    values::{
...
impl BlitzDomPainter<'_> {
    ...
    fn render_element(&self, scene: &mut impl PaintScene, node_id: usize, location: Point) {
        ...
        // `location` already has the page's top-level scroll offset baked
        // in (`paint_scene` starts the whole recursive walk at
        // `-viewport_scroll`, see below) -- correct for every ordinary,
        // in-flow node, but not for `position: fixed` ones: those were
        // just reparented by `BaseDocument::reparent_fixed_positioned_descendants`
        // (see its doc comment) to be direct children of the root
        // specifically so their layout offset is already root/viewport-
        // relative, and undoing the same scroll subtraction here is what
        // makes them actually stay pinned on screen as the page scrolls,
        // instead of moving away like an ordinary in-flow descendant.
        let box_position = if matches!(styles.clone_position(), StyloPosition::Fixed) {
            let viewport_scroll = self.dom.as_ref().viewport_scroll();
            box_position + Vec2::new(viewport_scroll.x, viewport_scroll.y)
        } else {
            box_position
        };
        let taffy::Layout {
